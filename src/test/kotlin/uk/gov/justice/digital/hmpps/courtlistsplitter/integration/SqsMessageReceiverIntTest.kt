...
import uk.gov.justice.digital.hmpps.courtlistsplitter.messaging.SqsMessageReceiver
...
class SqsMessageReceiverIntTest : IntegrationTestBase() {
  ...
  @Test
  fun `when message is received it is split into 4 messages which are published`() {
    ...
    val firstMessage = courtCaseEventsQueue?.sqsClient?.receiveMessage(
      ReceiveMessageRequest(courtCaseEventsQueue?.queueUrl),
    )
    firstMessage?.messages
    val sqsMessage: SqsMessageReceiver.Message = objectMapper.readValue(firstMessage?.messages?.get(0)?.body, SqsMessageReceiver.Message::class.java)
    val firstCase = objectMapper.readValue(sqsMessage.Message, CourtCaseEvent::class.java)
    assertThat(firstCase.caseNo).isEqualTo("1600032953")
  }
}
